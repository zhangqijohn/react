...
function DragSortingTable(props: DragSortingTablePropsType) {
    ...
    const originDateKeys =
        (originDate.length &&
            originDate.map((item: any) => {
                if (item.switch) {
    ...
    /* eslint-disable */
    useEffect(() => {
        if (props && props.colChange) props.colChange(data)
    }, [data])
    ...
    const resetSortTable = () => {
        const resetKeys: any[] = []
        const resetArr: ColumnPropsSwith[] =
            (originDate.length &&
                originDate.map(item => {
    ...
    const columns = [
        ...
        },
        {
            ...
            title: () => {
                return (
                    <span onClick={resetSortTable} style={{color: '#096dd9', cursor: 'pointer'}}>
                        重置
                    </span>
                )
            },
